...
class C:  # Parameter config
    ...
    RF = 4.2  # 4.5 [m] distance from rear to vehicle front end of vehicle
    RB = 0.7  # 1.0[m] distance from rear to vehicle back end of vehicle
    W = 2.0  # 3.0 [m] width of vehicle
    WD = 0.7 * W  # [m] distance between left-right wheels
    WB = 3.0  # 2.8[m] Wheel base
...
def design_obstacles(x, y, coverage):
    ox, oy = [], []
    a=20
    b=31
    #Main Map
    ...
    ctr=0
    while(ans<coverage):
        r_horizontal=randrange(250)
        r_vertical=randrange(250)
        obs_rand=randrange(3)
        if obs_rand==0:#horizontal obstacle
            if r_horizontal>234:
                r_horizontal=234
            for i in range(4*4):
                ox.append(r_horizontal+i)
                oy.append(r_vertical)
        elif obs_rand==1:# vertical obstacle
            if r_vertical>234:
                r_vertical=234
            for i in range(4*4):
                ox.append(r_horizontal)
                oy.append(r_vertical+i)
        elif obs_rand==2: # vertical Z obstacle
            if r_vertical<8:
                r_vertical=8
            if r_horizontal<1:
                r_horizontal=1
            for i in range(4*4):
                if i<8:
                    ...
                    ox.append(r_horizontal-1)
                    oy.append(r_vertical-i+7)
        ctr+=1
        ans=int(16*4*ctr/(250*250)*100)
...
def main():
    random.seed(10)
    print("start!")
    x,y=250,250 # Map size
    ox, oy = design_obstacles(x, y, 20)
    # print(ox)
    # print(oy)
    # plt.cla()
    # plt.plot(ox, oy, "sk")
    # plt.show()
    t0 = time.time()
    gx, gy, gyaw0 = 10.0, 40.0, np.deg2rad(0.0)
    g=[120.0,200.0,0.0,65.0,10.0,0.0,180.0,200.0,90.0]
    x1=[]
    y1=[]
    yaw1=[]
    direction1=[]
    for i in range(0,9,3):
        sx, sy, syaw0 = gx, gy, gyaw0 #Spawn point
        print(sx)
        gx, gy, gyaw0 = g[i], g[i+1], np.deg2rad(g[i+2])  #Goal
    ...
    for k in range(len(x1)):
        plt.cla()
        plt.plot(ox, oy, "sk", markersize=5)
        plt.plot(x1, y1, linewidth=1.5, color='r')
        if((time.time()-t3)/60>1):
            # print(time.time()-t3)
            draw_car(g[3], g[4], g[5], 0.0, 'red')
        if((time.time()-t3)/60>2):
            draw_car(g[6], g[7], g[8], 0.0, 'red')
        if k < len(x1) - 2:
            dy = (yaw1[k + 1] - yaw1[k]) / C.MOVE_STEP
            steer = rs.pi_2_pi(math.atan(-C.WB * dy / direction1[k]))
        else:
            steer = 0.0
        draw_car(g[0], g[1], g[2], 0.0, 'red')
