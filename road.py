...
iter1 = 0
while iter1 < n_ob:
    a = random.randrange(25, 210, 45)
    b = random.randrange(20, 210, 60)
    if (a,b) in ob_or:
        continue
    ob_or.append((a,b))
    iter1+=1
...
def main():
    ...
    while stime < n_min * 60:
        ...
        for k in range(len(x)):
            ...
            # plt.plot(bx, by, "bo", markersize=1, color = 'r')
        ...
        stime = stime//1
        stime += 1
        bx_total2.append(bx_total)
        by_total2.append(by_total)
        x_total2.append(x_total)
        y_total2.append(y_total)
        cx_total2.append(cx_total)
        cy_total2.append(cy_total)
        cyaw_total2.append(cyaw_total)
        csteer_total2.append(csteer_total)
        stimes2.append(stimes)
    for i in range(len(cx_total2)):
        for j in range(len(cx_total2[i])):
            plt.cla()
            plt.plot(ox, oy, "bo", markersize=1, color = 'darkgreen')
            plt.plot(bx_total2[i][j], by_total2[i][j], "bo", markersize=1, color = 'r')
            # plt.plot(x_total2[i], y_total2[i], linewidth=1.5, color='r')
            # draw_car(gx, gy, gyaw0, 0.0, 'dimgray')
            draw_car(cx_total2[i][j], cy_total2[i][j], cyaw_total2[i][j], csteer_total2[i][j])
            plt.axis("equal")
            strtime="{:.1f}".format(stimes2[i][j])
            plt.title("Simulation Time ="+strtime)
            # plt.xlabel("Simulation Time =",stimes2[i][j])
            plt.pause(0.0001)
