...
def calc_holonomic_heuristic_with_obstacle(sx, sy, node, ox, oy, reso, rr):
    n_goal = Node(round(node.x[-1] / reso), round(node.y[-1] / reso), 0.0, -1)

    wborder = 90

    lowerx = min(sx, node.x[-1]) - wborder
    lowery = min(sy, node.y[-1]) - wborder
    upperx = max(sx, node.x[-1]) + wborder
    uppery = max(sy, node.y[-1]) + wborder
    ...
    while True:
        ...
        for i in range(len(P.motion)):
            ...
            if node.x < lowerx or node.x > upperx or node.y < lowery or node.y > uppery:
                continue
